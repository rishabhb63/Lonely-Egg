...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == KSCAN_ESCAPE):  # for cross button
                pygame.quit()
                sys.exit()
                ...
                SCREEN.blit(GAME_SPRITES['background'], (0, 2))
...
def mainGame():
    ...
    playerJumped = False  # only true while Jumping
    ...
    while True:
        ...
        if crashTest:
            SCREEN.blit(GAME_SPRITES['crashed'], (playerx, playery))
            print('Crashed')
            pygame.display.update()
            FPSCLOCK.tick(FPS)
            return
        playerMidPos = playerx + GAME_SPRITES['player'].get_width() / 2  # Check Score
        ...
        for upperPipe, lowerPipe in zip(upperPipes, lowerPipes):  # move pipes to left
            upperPipe['x'] += pipeVelX
            lowerPipe['x'] += pipeVelX

        if 0 < upperPipes[0]['x'] < 5:  # add a new pipe when previous pipe go outside
            newpipe = getRandomPipe()
            upperPipes.append(newpipe[0])
            lowerPipes.append(newpipe[1])

        if upperPipes[0]['x'] < -GAME_SPRITES['pipe'][0].get_width():  # remove pipes out of Screen
            upperPipes.pop(0)
            lowerPipes.pop(0)

        SCREEN.blit(GAME_SPRITES['background'], (0, 0))  # Bliting of Background
        for upperPipe, lowerPipe in zip(upperPipes, lowerPipes):  # Bliting of Pipes
            SCREEN.blit(GAME_SPRITES['pipe'][0], (upperPipe['x'], upperPipe['y']))
            SCREEN.blit(GAME_SPRITES['pipe'][1], (lowerPipe['x'], lowerPipe['y']))

        SCREEN.blit(GAME_SPRITES['base'], (basex, GROUNDY))  # Bliting of Sprites
        SCREEN.blit(GAME_SPRITES['player'], (playerx, playery))  # Bliting of Player
...
def getRandomPipe():  # generate position of pipes
    pipeHeight = GAME_SPRITES['pipe'][0].get_height()
...
if __name__ == '__main__':
    ...
    pygame.display.set_caption('Lonely Egg')
    ...
    GAME_SPRITES['crashed'] = pygame.image.load(CRASHED).convert_alpha()
